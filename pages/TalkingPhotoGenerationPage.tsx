...
import { SCRIPTS, UI_TEXT, isEnglish } from '../lang';
...
const TalkingPhotoGenerationPage: React.FC<TalkingPhotoGenerationPageProps> = ({
  ...
  const [statusMessage, setStatusMessage] = useState(isEnglish() ? "Preparing talking photo generation..." : "말하는 사진 생성 준비 중...");
  ...
  useEffect(() => {
    if (!caricatureUrl) {
      setError(isEnglish()
        ? "Caricature not available. Please complete the previous step."
        : "캐리커처를 사용할 수 없습니다. 이전 단계를 완료해주세요.");
      setIsLoading(false);
      return;
    }
    ...
    if (!userData?.name) {
      setError(isEnglish()
        ? "User information not available. Please complete onboarding again."
        : "사용자 정보를 사용할 수 없습니다. 온보딩을 다시 완료해주세요.");
      setIsLoading(false);
      return;
    }
    ...
    if (!voiceId) {
      setError(isEnglish()
        ? "Voice information not available. Please complete voice recording again."
        : "음성 정보를 사용할 수 없습니다. 음성 녹음을 다시 완료해주세요.");
      setIsLoading(false);
      return;
    }
    ...
    const generate = async () => {
      ...
      try {
        ...
        if (result.isSample && result.message) {
          ...
          setStatusMessage(isEnglish() ? "Proceeding with sample video" : "샘플 영상으로 진행합니다");
        } else {
          setStatusMessage(SCRIPTS.talkingPhotoGenerated);
        }
        ...
        setError(isEnglish()
          ? "Failed to generate talking photo. This may be due to network or service issues. Please try again."
          : "말하는 사진 생성에 실패했습니다. 네트워크 또는 서비스 문제일 수 있습니다. 다시 시도해주세요.");
        setStatusMessage(isEnglish() ? "Error generating talking photo." : "말하는 사진 생성 중 오류 발생.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <PageLayout title={isEnglish() ? "Bringing Your Character to Life" : "캐릭터에 생명 불어넣기"}>
      <Card>
        {isLoading && (
          <div className="text-center py-10">
            ...
            <div className="bg-orange-50 rounded-lg p-4 mx-auto max-w-sm">
              <p className="text-orange-700 text-sm mb-2">{isEnglish() ? '🎬 Your character is coming to life!' : '🎬 캐릭터가 생생하게 살아나고 있어요!'}</p>
              <p className="text-slate-600 text-sm">{isEnglish() ? 'Please stay on this page' : '페이지를 나가지 마세요'}</p>
            </div>
          </div>
        )}
        ...
        {!isLoading && error && (
          <div className="text-center py-10">
            ...
            <Button onClick={() => setCurrentPage(Page.CaricatureGeneration)} variant="secondary" size="lg">
              {isEnglish() ? 'Back to Caricature' : '캐리커처로 돌아가기'}
            </Button>
          </div>
        )}
        {!isLoading && !error && generatedTalkingPhoto && (
          <div className="text-center">
            <h2 className="text-3xl font-semibold text-orange-600 mb-6">{isEnglish() ? 'Talking Character Complete!' : '말하는 캐릭터 생성 완료!'}</h2>
            {sampleVideoMessage && (
                ...
                {!videoError ? (
                    ...
                    <div className="w-64 h-96 md:w-80 md:h-[30rem] bg-gray-200 rounded-lg border-4 border-orange-500 flex flex-col items-center justify-center">
                        <p className="text-gray-600 text-center px-4">
                            {isEnglish() ? 'Unable to load video.' : '동영상을 로드할 수 없습니다.'}<br/>
                            <span className="text-sm">{isEnglish() ? 'May be processing on CDN.' : 'CDN에서 처리 중일 수 있습니다.'}</span>
                        </p>
                        <button
                            onClick={() => {
                                setVideoError(false);
                        ...
                        >
                            {isEnglish() ? 'Open direct link' : '직접 링크로 열기'}
                        </a>
                    </div>
                )}
            </div>
            <p className="text-base text-slate-600 mb-10">{isEnglish() ? "(How does it look? Your character can speak now!)" : "(어떤가요? 말하는 캐릭터가 생겼어요!)"}</p>
            ...
            <div className="flex justify-center">
              <Button
                ...
                size="lg"
              >
                {isEnglish() ? 'Learn About Deepfakes' : '딥페이크 이해하기'}
